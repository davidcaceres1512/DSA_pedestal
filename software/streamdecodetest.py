...
def on_message(client, userdata, msg):
    ...
    global elpos
    global elspeed
    azpos = AzPosition[len(AzPosition)-1]
    azspeed = AzSpeed[len(AzSpeed)-1]
    elpos = ElPosition[len(ElPosition)-1]
    elspeed = ElSpeed[len(ElSpeed)-1]
...
while True:
  if (time.process_time_ns()-publish_time)/1000000>=1000:
    ...
    publish_time = time.process_time_ns()
'''
  #if ((time.process_time_ns()-telegram_time)/1000000>=1000) and start_variable:
  #  if mqtt_status:
  #    MessageHandler.bot.send_message(chat_id="@PedestalJROBot", text="MQTT connected")
  #    MessageHandler.bot.send_message(chat_id="@PedestalJROBot", text="Last message was received "+str((time.process_time_ns()-start_time)/1000000)+" milliseconds ago.")
  #  else:
  #    MessageHandler.bot.send_message(chat_id="@PedestalJROBot", text="MQTT disconnected")
  #  MessageHandler.bot.send_message(chat_id="@PedestalJROBot", text="Azimuth position is "+str(azpos))
  #  MessageHandler.bot.send_message(chat_id="@PedestalJROBot", text="Azimuth speed is "+str(azspeed))
  #  MessageHandler.bot.send_message(chat_id="@PedestalJROBot", text="Elevation position is "+str(elpos))
  #  MessageHandler.bot.send_message(chat_id="@PedestalJROBot", text="Elevation speed is "+str(elspeed))
  #  telegram_time = time.process_time_ns()