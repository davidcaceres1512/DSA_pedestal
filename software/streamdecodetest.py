...
handler2 = logging.FileHandler('pedestal_mqtt_david_def.log', 'w', 'utf-8') # or whatever
...
def getSpeedPosition(msg_b64):
    ...
    Timestamp = (RawData[len(RawData)-8])|(RawData[len(RawData)-7]<<8)|(RawData[len(RawData)-6]<<16)|(RawData[len(RawData)-5]<<24)
    TimestampFractional = (RawData[len(RawData)-4])|(RawData[len(RawData)-3]<<8)|(RawData[len(RawData)-2]<<16)|(RawData[len(RawData)-1]<<24)
    counter = 0
    while counter < len(RawData)-2:
      ...
      if RawData[counter]==HEADER:
        ...
        counter+=1
        continue
    return RawData,AzPosition,AzSpeed,ElPosition,ElSpeed,Timestamp,TimestampFractional
...
def on_message(client, userdata, msg):
    ...
    RawData,AzPosition,AzSpeed,ElPosition,ElSpeed,Timestamp,TimestampFractional = getSpeedPosition(str(msg.payload.decode("utf-8")))
    ...
    with numpy.printoptions(precision=3, suppress=True):
        ...
        logging.debug("TimestampFractional is:"+str(TimestampFractional))
    azpos = AzPosition[-1]
    azspeed = AzSpeed[-1]
    elpos = ElPosition[-1]
    elspeed = ElSpeed[-1]
